...
function convert(source, dest, scale) {
    ...
    page.open(source, function (status) {
        ...
        try {
            ...
            // TODO-ci look at screen height also?
            if (dimensions.width > screenWidth && endsWith(dest, "pdf")) {
                scale = screenWidth / dimensions.width;
            }
...
function getSvgDimensions(page) {
    return page.evaluate(function () {
        ...
        var els, cnt = 1;
        if (!el.getBBox) {
            els = el.getElementsByTagName("SVG");
            el = els[0];
            cnt = els.length;
        }
        ...
        return { width: width, height: height * cnt, shouldScale: hasWidthOrHeight || !usesViewBox };
    });
}
